...
class DoubleRenderersTest {
    ...
    class NaturalRendererTests {
        ...
        void shouldRenderValuesWithDisplayPrecision(
            ...
            assertThat(render(5678.6578), equalTo(expected));
        }

        @ParameterizedTest
        @CsvSource(
                delimiter = '|',
                value = {"####.00000|5678.65780"})
        ...
        void shouldRenderValuesWithKmbtFormat(
                final byte displayPrecision, final double value, final String expected) {
    ...
    class PercentRendererTests {
        ...
        @CsvSource(
                delimiter = '|',
                value = {"0|567,866%", "1|567,865.8%", "2|567,865.78%"})
        void shouldRenderValuesWithDisplayPrecision(
                final byte displayPrecision, final String expected) {
            AttributeConstants.setDisplayPrecision(attrs, displayPrecision);
            assertThat(render(5678.6578), equalTo(expected));
    ...
    private SchemaField schemaField(final double value) {
        return SchemaField.schemaField(0, "test", field(value), metadata(attrs));
    }

    private static DoubleField field(final double value) {
        return row -> value;
    }
}
