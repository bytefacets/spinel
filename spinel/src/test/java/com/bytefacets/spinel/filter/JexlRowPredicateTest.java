...
import com.bytefacets.spinel.common.jexl.JexlRowContext;
...
class JexlRowPredicateTest {
    ...
    class BindingTests {
        ...
        void shouldCallbackWhenFieldNotFound() {
            ...
            final var ex = assertThrows(Exception.class, () -> predicate.bindToSchema(map::get));
    ...
    class FieldTypeTests {
        ...
        @Test
        void shouldHandleStringFieldNull() {
            final JexlRowPredicate predicate = jexlPredicate("b1 != null && b1 == \"ZIG\"");
            final Field b1 = field(TypeId.String, 1, new String[] {"bar", null, "ZIG"});
            final Map<String, Field> map = Map.of("b1", b1);
            predicate.bindToSchema(map::get);
            assertThat(predicate.testRow(0), equalTo(false));
            assertThat(predicate.testRow(1), equalTo(false));
    ...
    class StringMethodTests {
        ...
        @BeforeEach
        void setUp() {
            fieldMap.put("b1", field(TypeId.String, 1, new String[] {"bar", null, "ZIG"}));
        }

        @Test
        void shouldEvaluateToLowerCase() {
            final JexlRowPredicate predicate =
                    jexlPredicate("b1 != null && b1.toLowerCase() == \"zig\"");
            predicate.bindToSchema(fieldMap::get);
            assertThat(predicate.testRow(0), equalTo(false));
            assertThat(predicate.testRow(1), equalTo(false));
            assertThat(predicate.testRow(2), equalTo(true));
        }
        ...
        void shouldEvaluateStartsWith() {
            final JexlRowPredicate predicate = jexlPredicate("b1 != null && b1.startsWith(\"ZI\")");
            predicate.bindToSchema(fieldMap::get);
            assertThat(predicate.testRow(0), equalTo(false));
            assertThat(predicate.testRow(1), equalTo(false));
            assertThat(predicate.testRow(2), equalTo(true));
