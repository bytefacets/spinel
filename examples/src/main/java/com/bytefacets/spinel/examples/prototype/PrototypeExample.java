...
/**
 * This example demonstrates how a Prototype can be used to stabilize the effect of connection
 * resets. When a connection is reset, schemas are reset, too. This can be problematic for things
 * like UIs. When you use a Prototype, you trade off defining the schema locally for schema
 * stability for the consumers of the Prototype output. The prototype can also navigate some degree
 * of type casting, like an int field becoming a short: if your client says it's an int, but the
 * server sends a short, the prototype will cast (using {@link com.bytefacets.spinel.schema.Cast}).
 *
 * @see Prototype
